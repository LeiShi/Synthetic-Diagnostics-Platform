...
from ..settings.exception import GeometryError, GeometryWarning
...
class ExpGrid(Grid, metaclass=ABCMeta):
...
class Cartesian2D(ExpGrid):
    ...
    def __init__(self, **P):
        ...
        try:
            if ( 'DownLeft' in list(P.keys()) and 'UpRight' in list(P.keys()) ):
                ...
                if ( 'NR' in list(P.keys()) and not 'ResR' in list(P.keys()) ):
                    self.NR = P['NR']
                    self.ResR = rangeR / self.NR                
                elif ('ResR' in list(P.keys()) and not 'NR' in list(P.keys()) ):
                ...
                if ( 'NZ' in list(P.keys()) and not 'ResZ' in list(P.keys()) ):
                    self.NZ = P['NZ']
                    self.ResZ = rangeZ / self.NZ                
                elif ('ResZ' in list(P.keys()) and not 'NZ' in list(P.keys()) ):
                    # make sure the actual resolution is finer than the 
                    # required one
...
class Cartesian3D(ExpGrid):
    ...
    def __init__(self, **P):
        ...
        try:
            if ( ('Xmin' in list(P.keys())) and ('Xmax' in list(P.keys())) and ('Ymin' in \
                 list(P.keys())) and ('Ymax' in list(P.keys())) and ('Zmin' in list(P.keys())) \
                 and ('Zmax' in list(P.keys())) ):
                ...
                rangeZ = float(self.Zmax - self.Zmin)
                if ( 'NX' in list(P.keys()) and not 'ResX' in list(P.keys()) ):
                    self.NX = P['NX']
                    if(self.NX>1):
                        self.ResX = rangeX / (self.NX-1)
                    else:
                        self.ResX = 0
                elif ('ResX' in list(P.keys()) and not 'NX' in list(P.keys()) ):
                    # make sure the actual resolution is finer than the 
                    # required one
                    ...
                    sure you specify exactly one of them.')
                if ( 'NY' in list(P.keys()) and not 'ResY' in list(P.keys()) ):
                    self.NY = P['NY']
                    if(self.NY>1):
                        self.ResY = rangeY / (self.NY-1)               
                    else:
                        self.ResY = 0
                elif ('ResY' in list(P.keys()) and not 'NY' in list(P.keys()) ):
                    # make sure the actual resolution is finer than the 
                    # required one
...
class AnalyticGrid(Grid, metaclass=ABCMeta):
