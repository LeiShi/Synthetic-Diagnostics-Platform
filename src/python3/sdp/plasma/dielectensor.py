...
.. [4] :py:module:`..math.PlasmaDispersionFunction`
...
from ..math.pdf import Fq_list, F1q_list, F2q_list
from ..math.pdf import Z, a_pn
from .profile import PlasmaProfile
from ..settings.unitsystem import UnitSystem, cgs
from ..settings.exception import ModelInvalidError, ResonanceError, \
                                         PlasmaWarning
...
class HotSusceptibility(Susceptibility, metaclass=ABCMeta):
...
class SusceptNonrelativistic(HotSusceptibility):
    ...
    .. [1] "wave in Plasmas", Chapter 10-7, T.H.Stix, 1992, American Inst. of
           Physics 
    
    .. [2] :py:module:`..math.PlasmaDispersionFunction`
...
class SusceptRelativistic(HotSusceptibility):
    ...
    .. [1] :py:module:`..math.PlasmaDispersionFunction`
...
class Dielectric(object, metaclass=ABCMeta):
...
class ColdDielectric(Dielectric, metaclass=ABCMeta):
    ...
    def chi_i(self, coordinates, omega, k_para=None, k_perp=None, eq_only=True,
        ...
        if species_id is None:
            species_id = list(range(len(self.ion_species)))
        result = 0
        for i in species_id:
...
class WarmDielectric(Dielectric, metaclass=ABCMeta):
...
class HotDielectric(Dielectric, metaclass=ABCMeta):
