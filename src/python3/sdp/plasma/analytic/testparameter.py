...
def show_parameter2D():
    ...
    """
    for key,value in list(Parameter2D.items()):
...
def show_parameter1D():
    """Print out the parameters at the moment
    """
    for key,value in list(Parameter1D.items()):
...
are {}'.format(nshp, list(ShapeTable.keys())))
    profile['ne']= ne_array
    ...
    if ( tshp == 'exp'):
        ...
        raise KeyError('"{}" is not a valid shape code. Available shapes \
are {}'.format(tshp, list(ShapeTable.keys())))
    profile['Te']= Te_array
