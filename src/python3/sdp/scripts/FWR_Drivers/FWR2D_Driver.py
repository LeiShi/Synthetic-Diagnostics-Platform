...
#following names are default to be used in other scripts. DO NOT CHANGE THEM UNLESS YOU KNOW WHAT ELSE YOU NEED TO MODIFY. (list of dependent scripts:FWR_driver_input_file,Postprocess script using SDP.diagnostic.Reflectometry.FWR2D.Postprocess)
...
def start_run(params):
    ...
    with open('./output.txt','w') as output:
	    ...
	    print('Exception catched:{0}. from Case f={1},t={2},nc={3}. This run is skipped, others will continue, assuming this error is because you are trying to continue running the Script to finish a prematurely stopped run. Be sure this is the case to go on to use the result.'.format(str(e),f,t,nc), file=output)
	else:
	    print('Case f={0},t={1},nc={2} finished without exceptions.'.format(f,t,nc), file=output)
...
def main(argv):
    ...
    for opt,arg in opts:
        ...
        if(opt == '-t'):
            ...
            t_use = list(range(tstart,tend+1,tstep))
        if(opt == '-c'):
            nc_use = eval(arg)
        if(opt == '-n'):
