...
class BES:
    ...
    Look at the figure in :func:`compute_limits <sdp.diagnostic.bes.bes.BES.compute_limits>`, the two outer red lines for the limit.
    ...
    Look at the figure in :func:`get_solid_angle  <sdp.diagnostic.bes.bes.BES.get_solid_angle>`.
    ...
    :var self.beam: (:class:`Beam1D <sdp.diagnostic.beam.beam.Beam1D>`) Beam used for the diagnostic
    ...
    def get_bes(self):
        ...
        .. graphviz::
           ...
           Aemis [label="collision.get_emission"];
           Bemis [label="collision.get_emission"];
           ...
           subgraph cluster_lifetime { label="Beam1D.get_emis_lifetime"; Cint->Aquant->"collision.get_lifetime"->
        ...
        for i,time in enumerate(self.time):
            ...
            if self.multiprocessing:
                a = np.array(p.map(self.intensity_para, list(range(nber_fiber))))
                I[i,:] = a
            else:
    ...
    def get_solid_angle(self,pos,fib):
        ...
        * mixed case
        ...
        The two first are solved with the formula of Paxton (:func:`solid_angle_disk <sdp.math.Funcs.solid_angle_disk>`) and
