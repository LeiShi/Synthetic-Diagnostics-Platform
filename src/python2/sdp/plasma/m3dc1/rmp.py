    ...
    def generate_poincare(self, npsi_start=0, npsi_end=1, npsi=20, ntheta=1,
        ...
        for i, psi in enumerate(psi_plot):
            for j, theta in enumerate(theta_plot):
                result = odeint(_FL_prime, [psi, theta], zeta, args=(self,))
...
def _FL_prime(psi_theta, zeta, rmp_object, n=2):
...
def poincare_plot(rmp_object):
    color_table = ['r', 'b', 'm', 'g', 'k', 'c']
    psi_pc, theta_pc = rmp_object.poincare
    npsi = psi_pc.shape[0]
