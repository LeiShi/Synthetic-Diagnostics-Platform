...
def _FL_prime_c(psi_theta, zeta, rmp_object):
    ...
    g = rmp_object.F_interp(psi)
    I = rmp_object.I_interp(psi)
    dadt = rmp_object._dadt_c_re_interp(psi, theta) * np.cos(n*zeta) \
           - rmp_object._dadt_c_im_interp(psi, theta) * np.sin(n*zeta)
    dadz = -n*(rmp_object._alpha_c_im_interp(psi, theta) * np.cos(n*zeta) \
           + rmp_object._alpha_c_re_interp(psi, theta)*np.sin(n*zeta) )
    q = rmp_object.q_interp(psi)
