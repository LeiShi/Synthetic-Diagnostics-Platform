...
class RmpLoader(object):
    ...
    *******
    ...
    def generate_poincare(self, npsi_start=0, npsi_end=1, npsi=20, ntheta=1,
                          npoints=100, m='all', normalize=True, dview=None):
        ...
        :param bool normalize: if True, psi is normalized to psi_w after
                               calculation. Default to be True.i
        :param dview: direct view object provided by ipyparallel, for parallel
        runs.
        """
        ...
        if dview is None: # serial run
            result = map(lambda x: odeint(_FL_prime, x, zeta,
                                          args=(self,)),
                         zip(psi_all.flatten(), theta_all.flatten()))
        else:
            result = dview.map_sync(lambda x: odeint(_FL_prime, x, zeta,
                                                     args=(self,)),
                         zip(psi_all.flatten(), theta_all.flatten()))
...
def _FL_prime(psi_theta, zeta, rmp_object):
    ...
    gp = rmp_object.F_interp(psi, nu=1)
    Ip = rmp_object.I_interp(psi, nu=1)
    cosnzeta = np.cos(n*zeta)
    sinnzeta = np.sin(n*zeta)

    # alpha n harmonics are used twice later
    alpha_re_n = rmp_object.alpha_re_interp(psi, theta, grid=False)
    alpha_im_n = rmp_object.alpha_im_interp(psi, theta, grid=False)

    alpha = alpha_re_n*cosnzeta - alpha_im_n*sinnzeta
    # alpha derivatives can be evaluated from the fourier series formula
    dadt = ( rmp_object.dadt_re_interp(psi, theta, grid=False) * cosnzeta \
           - rmp_object.dadt_im_interp(psi,theta,grid=False)*sinnzeta )
    dadz = -n*(alpha_im_n*cosnzeta + alpha_re_n*sinnzeta )
    # psi derivatives is needed for accurately evaluate theta_dot
    dadp = rmp_object.alpha_re_interp(psi, theta, dx=1,
                                      grid=False)*cosnzeta\
         - rmp_object.alpha_im_interp(psi, theta, dx=1,
                                      grid=False)*sinnzeta

    q = rmp_object.q_interp(psi)

    dpsi_dzeta = (g*dadt - I*dadz)/q
    dtheta_dzeta = (1-dadp*g-alpha*gp)/(q+dadp*I+alpha*Ip)
...
def poincare_plot(poincare, coords='boozer',rmp=None, **keywords):
    """ Plot the Poincare plot based on the poincare data generated by
    rmp.generate_poincare.

    :param poincare: array of data generated by rmp.generate_poincare method
    :param string coords: optional, the plotting coordinates. Default to plot
        on Boozer coordinates.
        'RZ' plots on Cylindrical RZ coordinates.
        'geo' plots on geometric theta instead of boozer theta.
    :param rmp: optional, rmp object used when 'RZ' coordinates is used.
    ...
    if (coords == 'boozer'):
        ax.scatter(theta_pc, psi_pc, linewidth=0,
               c=psi_pc[:,0:1]+np.zeros_like(psi_pc), **keywords)
        plt.xlabel(r'$\theta_{Boozer}$ (rad)')
        plt.ylabel(r'$\psi/\psi_w$')
        plt.title("RMP Poincare Plot")
        plt.ylim(psi_pc.min(),psi_pc.max())
        plt.xlim(0, 2*np.pi)
    elif (coords == 'boozer-q'):
        # boozer theta vs. q
        try:
            # recover the absolute value of psi
            psi_pc *= rmp.psi_abs[-1]
            q_pc = rmp.q_interp(psi_pc)
        except KeyError as ke:
            print ('ERROR: RZ coordinates requires rmp object passed in with keyword:'
                   'rmp')
            raise ke
        ax.scatter(theta_pc, q_pc, linewidth=0,
               c=psi_pc[:,0:1]+np.zeros_like(psi_pc), **keywords)
        plt.xlabel(r'$\theta_{Boozer}$ (rad)')
        plt.ylabel(r'$q$')
        plt.title("RMP Poincare Plot")
        plt.xlim(0, 2*np.pi)
        plt.ylim(q_pc.min(), q_pc.max())

    elif (coords == 'RZ'):
        try:
            # recover the absolute value of psi
            psi_pc *= rmp.psi_abs[-1]
            r = rmp.R_interp(psi_pc, theta_pc, grid=False)
            z = rmp.Z_interp(psi_pc, theta_pc, grid=False)
        except KeyError as ke:
            print ('ERROR: RZ coordinates requires rmp object passed in with keyword:'
                   'rmp')
            raise ke
        ax.scatter(r, z, linewidth=0,
               c=psi_pc[:,0:1]+np.zeros_like(psi_pc), **keywords)
        plt.xlabel(r'$R$ (m)')
        plt.ylabel(r'$Z$ (m)')
        plt.title("RMP Poincare Plot")
        ...
        try:
            # recover the absolute value of psi
            r = rmp.R_interp(psi_pc*rmp.psi_abs[-1], theta_pc, grid=False)
            z = rmp.Z_interp(psi_pc*rmp.psi_abs[-1], theta_pc, grid=False)
            r0 = rmp.R_interp(rmp.psi_abs[0], 0)
            z0 = rmp.Z_interp(rmp.psi_abs[0], 0)
        except KeyError as ke:
            print ('ERROR: RZ coordinates requires rmp object passed in with keyword:'
                   'rmp')
            raise ke
        initial_psi = psi_pc[:,0:1]+np.zeros_like(psi_pc)
        theta_geo = np.arctan((z-z0)/(r-r0))
        region14 = r>=r0
        region23 = r<r0
        theta_geo_plus = theta_geo[region14]
        theta_geo_minus = theta_geo[region23]
        regionII = theta_geo_minus<0
        regionIII = theta_geo_minus>=0
        theta_geo_minus[regionII] += np.pi
        theta_geo_minus[regionIII] -= np.pi

        ax.scatter(theta_geo_plus,psi_pc[region14], linewidth=0,
               c=initial_psi[region14], **keywords)
        ax.scatter(theta_geo_minus,psi_pc[region23], linewidth=0,
               c=initial_psi[region23], **keywords)

        plt.xlabel(r'$\theta_{geo}$ (rad)')
        plt.ylabel(r'$\psi_n$')
        plt.title("RMP Poincare Plot")
        plt.xlim(-np.pi, np.pi)
        plt.ylim(psi_pc.min(), psi_pc.max())
    else:
        raise KeyError('ERROR: unknown coordiantes, use "boozer" or "RZ".')
