...
def get_coh_time(cha = channel):
    ...
    t_idx = np.argmax(std)
    print 'optimal window = {0:.4}'.format(time_array[t_idx])
    return time_array[t_idx]

def get_coh_median_std(cha = channel):
    window = get_coh_time(cha)

    cs_exp = nstx_exp.analyser.Coherent_over_time(0.632,0.640,1e-7,window,loader_num = cha)
    ...
    return np.median(cs_tophalf),np.std(cs_tophalf),cs_tophalf


    

def all_channel_coh_sigs():
    m = np.zeros((16))
    std = np.zeros((16))
    cs_sig = []

    for i in range(16):
        m,std,cs_tmp = get_coh_median_std(i)
