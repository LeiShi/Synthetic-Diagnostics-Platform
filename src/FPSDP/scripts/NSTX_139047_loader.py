...
def load(dimension,full_load,fluc_only):
    if dimension == 3:
        xgc_nstx_139047 = xgc.XGC_Loader(xgc_path,grid3D,time_start,time_end,time_step,dn_amplifier = 1,n_cross_section = 16, Full_Load = full_load, Fluc_Only = fluc_only)
    elif dimension == 2:
        xgc_nstx_139047 = xgc.XGC_Loader(xgc_path,grid2D,time_start,time_end,time_step,dn_amplifier = 1,n_cross_section = 16, Full_Load = full_load, Fluc_Only = fluc_only)
        
    return xgc_nstx_139047

if (__name__=='__main__'):
    xgc_nstx_139047 = load(True,True)
