...
def load(dimension,full_load,fluc_only,eq_only):
    if dimension == 3:
        xgc_nstx_139047 = xgc.XGC_Loader(xgc_path,grid3D,time_start,time_end,time_step,dn_amplifier = 1,n_cross_section = 1, Equilibrium_Only = eq_only,Full_Load = full_load, Fluc_Only = fluc_only)
    elif dimension == 2:
...
if (__name__=='__main__'):
    xgc_nstx_139047 = load(3,True,False,True)
    xgc_nstx_139047.cdf_output('/p/gkp/lshi/XGC1_NSTX_Case/new_3D_fluctuations/single_time/')
