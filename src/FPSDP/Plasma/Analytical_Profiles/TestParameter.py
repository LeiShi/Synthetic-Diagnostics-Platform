...
Parameter2D = { 'DownLeft':(-60,200), 'UpRight':(60,260),'NR':200, 'NZ':200, 
               'ne_0': 2e13, 'Te_0': 1*cgs['keV'], 'B_0': 20000, 
               'ne_shape': 'Hmode', 'Te_shape': 'Hmode', 'a': 50, 'R_0': 200,
               'dne_ne':0.01, 'dte_te':0.01, 'dB_B':0.01, 
               'timesteps':[0,1,2,3]}
...
def create_profile2D(random_fluctuation=False):
    ...
    if random_fluctuation:
        ...
        dne = 2*Parameter2D['dne_ne']*profile['ne']*random(d_shape)
        dTe = 2*Parameter2D['dne_ne']*profile['Te']*random(d_shape)
        dB = 2*Parameter2D['dB_B']*profile['B']*random(d_shape)
