...
*/
REAL **realAllocContiguous2d(size_t nrows,size_t ncols){
  REAL  **ptr = (REAL **)xmalloc(nrows*sizeof(REAL *));
  ptr[0] = (REAL *)xmalloc(nrows*ncols*sizeof(REAL));
...
*/
REAL ***realAllocContiguous3d(size_t npages,size_t nrows,size_t ncols){
  REAL  ***ptr = (REAL ***)xmalloc(npages*sizeof(REAL **));
  ptr[0] = (REAL **)xmalloc(npages*nrows*sizeof(REAL *));
  ptr[0][0] = (REAL *)xmalloc(npages*nrows*ncols*sizeof(REAL));
...
void realFreeDiscontiguous2d(int nrows,REAL ***array2d){
  int i;
  for(i=0;i<nrows;i++) free((*array2d)[i]);
  free(*array2d);
  *array2d = NULL;
}
...
void realFreeDiscontiguous3d(int npages,int nrows,REAL ****array3d){
  int i,j;
  for(i=0;i<npages;i++){
    for(j=0;j<nrows;j++) free((*array3d)[i][j]);
    free((*array3d)[i]);
  }
  free(*array3d);
  *array3d = NULL;
