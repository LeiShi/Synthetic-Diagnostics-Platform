...
// files
#define PSI_GRID_FNAME "NTprofile.cdf" // used by get_fluctuations
#define PHI_DATA_DIR "./data1/"
#define FLUC_FNAME "fluctuations.cdf" // used by fluc_info_file
...
int fluc_info_file();
int get_fluctuations(int npts,size_t nTimeSteps,REAL phi[],REAL a[],REAL theta[],REAL zeta[],int timeSteps[],int flag[]);
...
int get_fluctuations(int npts,size_t nTimeSteps,REAL phi[],REAL a[],REAL theta[],REAL zeta[],int timeSteps[],int flag[]){
  ...
  int status = readAllPhiFiles(PHI_DATA_DIR,nTimeSteps,timeSteps,phi_ints,phi_reals,&igrid,
...
//#endif
  ...
  /* REAL *psi_grid=(REAL*) xmalloc(sizeof(REAL)*mpsi);
  int idx;
